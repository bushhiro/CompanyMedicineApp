...
class ExemptionFormDialog extends StatefulWidget {
  ...
  const ExemptionFormDialog({super.key, required this.patientId});
...
class _ExemptionFormDialogState extends State<ExemptionFormDialog> {
  ...
  Future<void> _saveExemption() async {
    if (_vaccineType == null || _date == null || _numberController.text.isEmpty || _pickedImage == null) {
    ...
    try {
      final uri = Uri.parse('http://192.168.29.112:65322/api/v1/vaccines/withdrawals');
      ...
      request.fields.addAll({
        ...
        'med_withdrawl_num': _numberController.text,
      });
      ...
      // DEBUG
      debugPrint("=== DEBUG EXEMPTION UPLOAD ===");
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Медотвод успешно добавлен")),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Ошибка при добавлении медотвода: $respStr")),
        );
      }
    } catch (e, stack) {
      debugPrint("ERROR: $e");
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Ошибка при добавлении медотвода: $e")),
      );
    }
  }
}